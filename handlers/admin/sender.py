...
from states import SenderStates
...
async def get_ids():
    return [i[0] for i in (await User.get_all())]
...
def init_handlers():
    @dp.callback_query(F.data == 'start_send_messages')
    async def start_sender(call: CallbackQuery, state: FSMContext):
        ...
        await call.message.answer(
            ...
            reply_markup=SenderKeyboards.without_text().as_markup()
        )
